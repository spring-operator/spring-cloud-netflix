...
/*
	@Autowired
	private SimpleHostRoutingFilter filter;
	...
	public void contentLengthTest(Long expectedContentLength, SimpleHostRoutingFilter filter, MockMultipartHttpServletRequest request) throws IOException {
		...
		CloseableHttpResponse httpResponse = (CloseableHttpResponse) RequestContext.getCurrentContext()
				.get("zuulResponse");
		assertThat(httpResponse.getStatusLine().getStatusCode(), equalTo(200));
		InputStream zuulResponse = httpResponse.getEntity().getContent();
		assertThat(zuulResponse, not(instanceOf(EmptyInputStream.class)));
